...
$1 == "ip" {
	...
	if ((value != "") && (value !~ /^[[:digit:]]{1,3}\.[[:digit:]]{1,3}\.[[:digit:]]{1,3}\.[[:digit:]]{1,3}$/)) valid = 0
		...
		if ((ipaddr[4] == 0) || (ipaddr[4] == 255)) valid = 0
		#block 127.0.0.0 to 127.255.255.255 (loopback addresses used to test TCP/IP)
		if (ipaddr[1] == 127) valid = 0
		#block 169.254.0.0 to 169.254.255.255 (windows automatic IP addresses)
		if ((ipaddr[1] == 169) && (ipaddr[2] == 254)) valid = 0
		#block 240.0.0.0 to 255.255.255.255 (class E address reserved for experimental use)
		if (ipaddr[1] >= 240) valid = 0
	}
...
$1 == "netmask" {
	...
	if ((value != "") && (value !~ /^[[:digit:]]{1,3}\.[[:digit:]]{1,3}\.[[:digit:]]{1,3}\.[[:digit:]]{1,3}$/)) valid = 0
		...
		if ((ipaddr[1] == 255) && (ipaddr[2] == 255) && (ipaddr[3] == 255) && (ipaddr[4] == 255)) valid = 0
		if (valid != 0) {
