...
$1 ~ /^button/ {
	...
	if ($4 == "hidden") field_opts = field_opts " style=\"display: none\""
	#if ($5 == "disabled") opts = " disabled=\"disabled\""
	print "<tr" field_opts ">"
	#if ($2 != "") print "<td width=\"40%\">" $2 "</td><td width=\"60%\">" else print "<td colspan=\"2\">"
	print "<td colspan=\"2\">"
...
$1 ~ /^radio/ {
	...
	if ($6 == "disabled") opts = opts  " disabled=\"true\""
	if (onchange != "") opts = opts " onchange=\"" onchange "(this)\""
	if (onclick != "") opts = opts " onclick=\"" onclick "(this)\""
	print "<input type=\"radio\" id=\"" $2 "\" name=\"" $2 "\" value=\"" $4 "\" " opts " />"
}
$1 ~ /^select/ {
	opts = ""
	if (onchange != "") opts = opts " onchange=\"" onchange "(this)\""
...
$1 ~ /^password/ { 
    if ($5 == "disabled") opts = " disabled=\"true\""
    if ($5 != "")
	print "<input id=\"" $2 "\" type=\"password\" name=\"" $2 "\" value=\"" $3 "\" "opts"/>" $4 
    else
	print "<input id=\"" $2 "\" type=\"password\" name=\"" $2 "\" value=\"" $3 "\" />" $4 
}
...
$1 ~ /^submit/ { 
    if ($4 == "disabled") opts = " disabled=\"true\""
    if ($4 != "")
	print "<input type=\"submit\" id=\"" $2 "\" name=\"" $2 "\" value=\"" $3 "\" "opts" />" 
    else
	print "<input type=\"submit\" id=\"" $2 "\" name=\"" $2 "\" value=\"" $3 "\" />"
}
