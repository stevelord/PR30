...
static void sdhci_prepare_data(struct sdhci_host *host, struct mmc_data *data)
{
	...
	if (unlikely((host->flags & SDHCI_REQ_USE_DMA) &&
	 ...
	 if (host->flags & SDHCI_BAD_DMA ) host->flags &= ~SDHCI_REQ_USE_DMA;
...
static void sdhci_send_command(struct sdhci_host *host, struct mmc_command *cmd)
{
	...
	unsigned long timeout;
        ...
        if ( host->id == 1 )
            pr_debug(KERN_INFO "%s 0x%x is starting...\n", __func__ , cmd->opcode);
...
#ifdef CONFIG_TIWLAN_SUPPORT
#define	TIWLAN_MMC_PORT 0
       if ( pdev->id == TIWLAN_MMC_PORT &&  mmc_plat )
       {
           pr_debug(KERN_INFO "%s: id[%d] pata:%p embedded_sdio:%p\n",__func__, pdev->id,mmc_plat,mmc_plat->embedded_sdio);
             if ( mmc_plat->embedded_sdio ) {
             host->flags |= SDHCI_CD_PRESENT;
          	pr_debug(KERN_INFO "%s: pdata->embedded_sdio->cis vendor[%08X] device[%08X] blksize:%d\n",__func__ ,

             mmc_plat->embedded_sdio->cis.vendor,
             mmc_plat->embedded_sdio->cis.device,
             mmc_plat->embedded_sdio->cis.blksize);
             mmc_set_embedded_sdio_data(mmc,
             &mmc_plat->embedded_sdio->cis,
             &mmc_plat->embedded_sdio->cccr,
             mmc_plat->embedded_sdio->funcs,
             mmc_plat->embedded_sdio->num_funcs);
             tiwlan_set_mmc_host(mmc);
               }
      }
#endif
