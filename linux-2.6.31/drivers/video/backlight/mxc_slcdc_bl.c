/*
 * Copyright 2009 BandRich, Inc. All Rights Reserved.
 */
 ...
 * @file mxc_slcdc_bl.c
 *
 * @brief Backlight Driver for SLCDC on Freescale MXC/i.MX platforms.
 *
...
#define MXC_DEFAULT_INTENSITY 	128
...
extern u8 mx2fb_get_brightness( void );
...
{
#if 1
	if (strcmp(info->fix.id, "SLCDC") == 0) {
		return 1;
	}
	TRACE("%s():fix.id -%s incorrect", __FUNCTION__, info->fix.id );
#else // original	
...
//	devdata->clk = clk_get(NULL, "lcdc_clk");
	...
	bd->props.brightness = mx2fb_get_brightness();
	bd->props.max_brightness = MXC_MAX_INTENSITY;
	bd->props.power = FB_BLANK_UNBLANK;
	bd->props.fb_blank = FB_BLANK_UNBLANK;
//	mx2fb_set_brightness(MXC_DEFAULT_INTENSITY);

	INFO("%s():MXC Backlight Device %s Initialized.", __FUNCTION__, dev_name(&pdev->dev));
	return 0;
      err0:
	kfree(devdata);
	return ret;
