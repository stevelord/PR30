...
#include <linux/timer.h>
	...
	#define BR_FEATURE_UDC_FIX_USB_RECOGNIZE_FS_DEV

	#define BR_FEATURE_UDC_DEBUG_MSG
#endif
...
#ifdef BR_FEATURE_UDC_DEBUG_MSG 
	pr_debug("%s():enable=%d, may_wakeup=%d\n", __FUNCTION__, enable, device_may_wakeup(udc_controller->gadget.dev.parent) );
#endif
...
#if 1 
extern void BR_extra_key_report_event( unsigned int code );
extern void otg_switch_report_event( unsigned int code );

//static bool g_otg_is_suspend = 0;
static struct timer_list suspend_timer;
static int start_suspend_timer_flag = 0;	//0 is stop, 1 is start
#endif
...
#if 1
static int check_allow_sleep( void )
{
	int allow_sleep = 1;
 ...
 */
static ssize_t show_allow_sleep(struct device *dev, struct device_attribute *attr, char *buf)
{
...
static DEVICE_ATTR(allow_sleep, S_IRUGO, show_allow_sleep, NULL);

void BR_otg_device_create_file( void )
{
	int	status = -EINVAL;
	
	status = device_create_file(&(udc_controller->gadget.dev), &dev_attr_allow_sleep);
	if (status != 0)
	{
...
void BR_otg_device_remove_file( void )
{
	device_remove_file(&(udc_controller->gadget.dev), &dev_attr_allow_sleep);
}
#endif

#if 1 
static void suspend_timer_start( unsigned long expires )
{
	start_suspend_timer_flag = 1;

	mod_timer(&suspend_timer, expires );
}

static void suspend_timer_func(unsigned long args)
{
//	u32 temp;

	if ( start_suspend_timer_flag )
	{
//		temp = fsl_readl(&dr_regs->portsc1);
//		printk("%s():portsc1=0x%x, function=%s\n", __FUNCTION__, temp, udc_controller->driver->function );

		if ( check_allow_sleep() )
		{
			BR_extra_key_report_event( KEY_SLEEP );

			if ( udc_controller->driver->function )
			{
				if ( !strcmp(udc_controller->driver->function, "g_ether") ) 
					otg_switch_report_event( SW_ECM_TO_CDROM );
			}
		}

#if 0
		if ( !(temp & PORTSCX_CURRENT_CONNECT_STATUS) )
		{
			if ( udc_controller->driver->function )
			{
				if ( !strcmp(udc_controller->driver->function, "g_ether") ) 
...
static void init_suspend_timer(void)
{
	init_timer(&suspend_timer);
	suspend_timer.expires = jiffies + HZ / 2;
	suspend_timer.data = 0;
	suspend_timer.function = suspend_timer_func;

	start_suspend_timer_flag = 0;
}

static void del_suspend_timer(void)
{
	start_suspend_timer_flag = 0;
...
#ifdef BR_FEATURE_UDC_FIX_USB_RECOGNIZE_FS_DEV 
...
static void dr_phy_low_power_mode(struct fsl_udc *udc, bool enable)
{
	...
	pr_debug("%s():may_wakeup=%d, enable=%d\n", __FUNCTION__, 
			device_can_wakeup(udc_controller->gadget.dev.parent), enable );
#endif
...
#ifdef BR_FEATURE_UDC_FIX_USB_RECOGNIZE_FS_DEV
	pr_debug("%s():Set the controller as device mode\n", __FUNCTION__ );
		...
		unsigned int zlt, unsigned char mult)
{
	...
	case USB_ENDPOINT_XFER_INT:
		...
		break;
	default:
		ERR("error ep type is %d", ep_type);
		return;
	}
	if (zlt)
...
static int fsl_ep_disable(struct usb_ep *_ep)
{
	...
	ep = container_of(_ep, struct fsl_ep, ep);
	if (!_ep || !ep->desc) {
		ERR("%s not enabled", _ep ? ep->ep.name : NULL);
		return -EINVAL;
	}
#ifdef BR_FEATURE_UDC_DEBUG_MSG 
	pr_debug("%s():ep.name=%s\n", __FUNCTION__, ep->ep.name);
...
#ifdef BR_FEATURE_UDC_DEBUG_MSG
	pr_debug("%s():remote_wakeup=%d\n", __FUNCTION__, udc->remote_wakeup );
#endif
...
#if 1
//	pr_debug("%s():USB_STATE_SUSPENDED\n", __FUNCTION__ );

//	if ( !g_otg_is_suspend )
	{
		suspend_timer_start( jiffies + 2 * HZ  );  // jiffies + HZ/2
	}
#endif
...
{
#ifdef BR_FEATURE_UDC_DEBUG_MSG
	pr_debug("%s():usb_state=%d, portsc1=0x%x\n", __FUNCTION__, udc->resume_state, 
...
{
#ifdef BR_FEATURE_UDC_DEBUG_MSG
	pr_debug("%s():is_otg=%d, usb_state=%d, USB_ID=0x%x\n", __FUNCTION__, 
