				...
				pm_message_t message)
{
	...
	if (!device_may_wakeup(&(pdev->dev))) {
#if 1
		pr_debug("%s():can_wakeup=%d, should_wakeup=%d\n", __FUNCTION__, pdev->dev.power.can_wakeup, pdev->dev.power.should_wakeup );
#else
...
FUNCTION     BR_ehci_restore_registers()

DESCRIPTION
  This function will restore EHCI registers.
...
===========================================================================*/
void BR_ehci_restore_registers( struct usb_hcd *hcd )
{
	struct fsl_usb2_platform_data *pdata;
	struct ehci_hcd *ehci = hcd_to_ehci(hcd);
	...
	if ( !hcd )
	{
		printk(KERN_WARNING "%s(): hcd=NULL\n", __FUNCTION__ );
		return;
	}
	...
	if ( !pdata )
	{
		printk(KERN_WARNING "%s(): pdata=NULL\n", __FUNCTION__ );
		return;
	}
