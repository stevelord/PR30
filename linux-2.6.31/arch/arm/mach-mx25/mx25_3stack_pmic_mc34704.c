...
*/
#if 1
static struct regulator_init_data oled_init = {
	.constraints = {
			.name = "REG8_OLED",
			.min_uV =
			mV_to_uV(REG8_V_MV * (1000 + REG8_DVS_MIN_PCT * 10) /
				 1000),
			.max_uV =
			mV_to_uV(REG8_V_MV * (1000 + REG8_DVS_MAX_PCT * 10) /
				 1000),
			.valid_ops_mask = REGULATOR_CHANGE_VOLTAGE,
			}
};
...
*/
#if 1
	mc34704_register_regulator(mc34704, MC34704_REG8, &oled_init);
#endif
...
*/
#if 1
	return i2c_register_board_info(1, &mc34704_i2c_device, 1);
#else
