...
static int wpa_driver_tista_driver_cmd( void *priv, char *cmd, char *buf, size_t buf_len )
{
	...
	if( os_strcasecmp(cmd, "start") == 0 ) {
		...
		if( ret == 0 ) {
			len = wpa_driver_wext_get_ssid( priv, (u8 *)ssid );
					   ...
					   void *ctx, char *data, int len)
{
	...
	wpa_hexdump(MSG_MSGDUMP, "wext event", data, len);
	...
	while (pos + IW_EV_LCP_LEN <= end) {
		...
		wpa_printf(MSG_DEBUG, "WEXT: wpa_driver_wext_event_wireless cmd 0x%x", iwe->cmd);
