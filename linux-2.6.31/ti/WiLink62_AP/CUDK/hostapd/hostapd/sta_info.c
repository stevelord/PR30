...
void ap_free_sta(struct hostapd_data *hapd, struct sta_info *sta)
{
	...
	//if ( 0 == strcmp(hapd->conf->iface, BR_HOSTAPD_WIFI_IFNAME) )
	{
		all_info_t *all_info = NULL;

		if ( 0 == BR_hostapd_shm_open(&all_info) )
		{
			all_info->u_wifi.num_sta = hapd->num_sta;
			BR_hostapd_shm_close();
		}

		// Inform BRLCD if the number of STA reaches 0
		if (0 == hapd->num_sta)
		{
			system("kill -s USR2 `cat /var/lock/brlcd.lock`");
		}
	}
